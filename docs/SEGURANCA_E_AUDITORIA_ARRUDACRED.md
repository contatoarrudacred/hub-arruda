...
**Revisado em 17/08/2026 (conferido direto contra as migrations, este trecho estava desatualizado em mais de um ponto).** Trigger de auditoria hoje cobre 17 das ~27 tabelas do projeto (contagem aproximada — o número de tabelas cresce a cada migration, ver `supabase/migrations/`): `etapas_fluxo`, `fluxos`, `usuarios_sistema`, `pessoas`, `conversas`, `oportunidades` (as 6 originais), `faqs`, `objecoes`, `precos_por_faixa`, `configuracoes`, `agendas_followup`, `agenda_itens` (essas 6 já tinham sido cobertas em 15/08/2026, ao construir os respectivos CRUDs — este documento nunca tinha sido atualizado pra refletir isso), `mensagens`/`followup_emails`/`cliques_rastreio`/`produtos` (adicionadas em 16/08/2026), e **`notas_internas`/`respostas_prontas`** (adicionadas em 17/08/2026, junto com o Bloco B da Tela de Atendimento). **`notificacoes` e `cron_locks`** (também novas em 16-17/08/2026) foram deixadas de propósito **sem** trigger de auditoria — são estado operacional efêmero (notificação lida/não lida, lock de execução do cron), não registro de negócio; mesmo raciocínio já usado pra não auditar sessões/cache.
...
**Continua pendente:**
- ~~`produtos` tem RLS mas não tem trigger de auditoria~~ ✅ corrigido, migration [`20260816040000_avaliacao_quick_wins.sql`](../supabase/migrations/20260816040000_avaliacao_quick_wins.sql) já rodada no Supabase.
