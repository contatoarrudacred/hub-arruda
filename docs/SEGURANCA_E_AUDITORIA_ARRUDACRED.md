...
**Revisado em 16/08/2026 (conferido direto contra as migrations, este trecho estava desatualizado em mais de um ponto).** Trigger de auditoria hoje cobre 15 das 23 tabelas do projeto: `etapas_fluxo`, `fluxos`, `usuarios_sistema`, `pessoas`, `conversas`, `oportunidades` (as 6 originais), `faqs`, `objecoes`, `precos_por_faixa`, `configuracoes`, `agendas_followup`, `agenda_itens` (essas 6 já tinham sido cobertas em 15/08/2026, ao construir os respectivos CRUDs — este documento nunca tinha sido atualizado pra refletir isso), e **`mensagens`/`followup_emails`/`cliques_rastreio`** (adicionadas em 16/08/2026).
