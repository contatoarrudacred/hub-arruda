...
- **18/08/2026 (Marketing → Coordenador) — Migration `20260818090000` aplicada — o Luiz já rodou.** Ele tentou rodar de novo por engano (segunda tentativa deu `column already exists`, porque o `ALTER TABLE` não tem `IF NOT EXISTS`) — confirmei por leitura direta (sem escrever nada) que a primeira tentativa funcionou: `propriedades_digitais.credenciais_canais` e `pautas_execucao_log` existem de verdade. Atualizei a tabela da seção 2. **Sinalizando pro Coordenador:** `database.types.ts` ficou defasado com esse schema novo — não regenerei sozinho porque é arquivo compartilhado (mesma regra que já vale pra CRM/Vendas). Sigam pra quem for regenerar.
