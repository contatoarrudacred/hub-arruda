...
- **`contratos`** (nova): `id`, `oportunidade_id` (FK, único — reforça a regra já fechada em `KANBAN_COMERCIAL_LIMPANOME.md`/seção 11 do plano mestre: **1 Oportunidade = 1 contrato, mesmo em pacote de vários documentos**), `contrato_template_id` (FK), `pessoa_signatario_id` (FK → `pessoas` — o cliente, ou o representante legal via `pessoa_representantes` quando PJ), `pessoa_arrudacred_signatario_id` (FK → `pessoas` — configurável, não hardcoded), `fornecedor_id` (FK → `pessoas`, nullable — só preenchido quando o Produto é `subcontratado` **e** `fornecedor_definido_em = 'venda'`; é aqui que a escolha por venda fica registrada, não em `produtos`, ver 3.1), `pdf_url` (Supabase Storage), `status` (`'gerado' | 'enviado' | 'assinado' | 'recusado' | 'cancelado'`), `assinafy_document_id`, `forma_pagamento` (`'avista' | 'parcelado'`), `metodo_pagamento` (novo — `'boleto' | 'cartao' | 'voucher' | 'outro'`), `parcelas_qtd` (`1` = à vista), `valor_total` (snapshot de `oportunidades.valor_estimado` no momento da geração — inclui a soma de `oportunidade_documentos` quando é pacote), `enviado_em`, `assinado_em`, `created_at`, `updated_at`. Trigger de auditoria + RLS.
  - **Correção importante (achado ao conferir o código real, e decisão tomada com Luiz em 18/08/2026):** `forma_pagamento` **não é calculado por Vendas nem lido de `precos_por_faixa`** — `precos_por_faixa` é território exclusivo do CRM/atendimento, usado pela Malala pra apresentar preço ao lead (Passo 15 do script), e esta spec não mexe nele. Hoje o checkpoint `ln_passo15_normal`/`ln_passo15_selfservice` (`src/lib/motor-fluxo/fluxo-limpeza-nome.ts`) só captura o binário `avista`/`parcelado` em `conversas.dados` — **não captura** `metodo_pagamento`, quantidade de parcelas, valor de cada parcela nem datas de vencimento. Não dá pra gerar contrato/tabela de vencimentos só com isso. Pedido de captura completa nativa no bot já registrado pro CRM em `docs/COORDENACAO_AGENTES_ARRUDACRED.md` (seção 3, 18/08/2026) — é melhoria de médio prazo, não bloqueia esta frente.
  - **Solução desta frente: tela de "Fechamento de Venda" (ver 3.2.1)** — completa o que falta no momento de gerar o contrato, tanto pra Oportunidade vinda do funil do CRM quanto pra venda sem funil prévio. `forma_pagamento` de `conversas.dados` é usada só como **pré-preenchimento** (quando existir) — o admin sempre confirma/completa antes do contrato ser gerado.
...
- **`contrato_parcelas`** (nova): `id`, `contrato_id` (FK), `numero`, `valor`, `vencimento_previsto`, `asaas_payment_id` (nullable), `status` (`'previsto' | 'gerado' | 'pago' | 'atrasado' | 'cancelado'`), `pago_em` (nullable), `created_at`, `updated_at`. O plano (quantidade/valor de cada parcela/vencimentos) vem do que foi confirmado na tela de Fechamento de Venda (3.2.1) — transcrito em linhas individuais pra alimentar a tabela de vencimentos do PDF e depois ser **reaproveitado**, sem recalcular, quando a cobrança real é criada na Asaas após a assinatura. Trigger de auditoria + RLS.
...
### 3.2.1 Tela de Fechamento de Venda (passo de UI, sem entidade nova)

- **Não é tabela nova** — é uma tela/passo do fluxo de Vendas, encaixado logo antes da geração do contrato, sobre os campos já modelados em `contratos`/`contrato_parcelas` acima. Existe pra fechar, agora, a lacuna real: hoje **nenhuma origem de venda** entrega 100% do que o contrato precisa sobre pagamento.
- **Aparece nos dois caminhos de entrada em `dados_contrato`** (seção 1):
  1. **Oportunidade vinda do funil do CRM:** pré-preenche `forma_pagamento` com o que já está em `conversas.dados` (avista/parcelado), se existir — o admin confirma e completa o resto (`metodo_pagamento`, quantidade de parcelas quando parcelado, valor de cada parcela, data de vencimento de cada uma).
  2. **Venda sem funil prévio:** mesma tela, sem pré-preenchimento (não existe `conversas.dados` pra puxar) — o admin preenche tudo do zero, junto com Pessoa + Produto + valor (seção 3.1).
- **Validação:** soma das parcelas informadas precisa bater com `valor_total`; à vista (`parcelas_qtd = 1`) não pede tabela de vencimentos, só a data de pagamento único.
- **Resultado:** grava `contratos.forma_pagamento`/`metodo_pagamento`/`parcelas_qtd` e as linhas de `contrato_parcelas` — só então o botão de gerar contrato fica disponível.
- **Efêmero por design:** quando o CRM passar a capturar esse detalhe nativamente no bot (pedido em aberto, ver acima), esta tela passa a nascer 100% pré-preenchida pra Oportunidades vindas do funil (o admin só confirma, não digita) — mas continua existindo do mesmo jeito pra venda sem funil prévio, que nunca passa pelo bot. Não é retrabalho: o desenho já separa "captura" (aqui) de "origem do dado" (CRM ou digitação manual).
