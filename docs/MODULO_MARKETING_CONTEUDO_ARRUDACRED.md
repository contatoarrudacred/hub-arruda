...
- **Fluxo completo (persona → post distribuído), mapeado com o Luiz em 19/08/2026:** diagrama + estado atual de cada etapa em [claude.ai/code/artifact/48dcf7b3-1b5c-45c9-96b5-28ac4bc8aef0](https://claude.ai/code/artifact/48dcf7b3-1b5c-45c9-96b5-28ac4bc8aef0) — validado pelo Luiz, é a referência visual das três sub-fases abaixo.
- **Fase 4a — YMYL/E-E-A-T/AEO-GEO, auditoria de 19/08/2026** (`docs/insumos-externos/auditoria_seo_aeo_geo_llm_post.md`, análise de uma amostra real gerada pelo pipeline de personas). Diagnóstico: SEO on-page/AEO já está forte (9/10 em várias dimensões — H2 em pergunta, resposta direta, FAQ), mas confiabilidade factual YMYL (5/10), E-E-A-T (6/10) e originalidade em escala (6/10) precisam de camada nova. O que **já existe** no checklist de 11 itens (seção 5.2) cobre a parte estrutural (H1, intro, FAQ+schema, links, meta tags, cobertura semântica). O que **não existe ainda**:
  - Gate de precisão factual YMYL no Revisor — hoje ele só pontua contra o checklist genérico; precisa de uma dimensão própria pra afirmações jurídicas/financeiras sensíveis (ex.: não confundir prescrição de dívida com prazo de negativação — achado confirmado na prática 2x no teste real de 19/08), com regra de que uma afirmação YMYL sem lastro reprova sozinha, independente do resto do score. **Isto estende a mesma etapa/decisão do Revisor que já existe — não é uma etapa nova depois da aprovação** (esclarecido com o Luiz em 19/08, o desenho inicial do fluxo tinha isso errado).
  - **Sugestão de correção na reprovação (pedido do Luiz, 19/08/2026)** — hoje o Revisor só grava `motivo_ultima_reprovacao` (o que está errado). Passa a também gravar uma sugestão concreta de como o texto poderia mudar pra ser aprovado — dá pro Escritor automático um alvo mais claro na próxima tentativa (além do rascunho anterior + motivo, que já existe), e principalmente ajuda o **revisor humano** quando uma pauta é bloqueada depois de 3 tentativas (`marcarPautaBloqueada`) — hoje ele só vê o motivo da última reprovação, sem direção de correção.
  ...
  - **Autoria/E-E-A-T resolvida no cadastro da Propriedade (decidido com o Luiz, 19/08/2026)** — em vez de escolher autor por post, cada propriedade digital registra uma vez (tela Propriedades Digitais, Fase 2) os dados de autoria exigidos pela auditoria (seção 12 de `auditoria_seo_aeo_geo_llm_post.md`): nome real, foto, experiência, especialidade, empresa, credenciais, perfis profissionais. Todo post publicado daquela propriedade herda automaticamente esses dados (autor, e opcionalmente revisor) — precisa de colunas novas em `propriedades_digitais` (migration) e de os campos aparecerem no adaptador WordPress e no schema `Article` (item abaixo). Datas de publicação/atualização continuam por post (`created_at`/`updated_at` já existem em `posts`), não por propriedade.
  - Dados estruturados `Article`/`Organization` (schema.org) — o Adaptador WordPress não gera isso hoje, só o `FAQPage` embutido pelo próprio Escritor. O campo `author` do `Article` vem da autoria de propriedade (item acima).
  ...
  - Regras leves de precisão devem entrar no prompt do **Escritor** (não afirmar prazo jurídico sem fonte, não fundir conceitos distintos); a auditoria completa por dimensão (SEO/AEO-GEO/YMYL/E-E-A-T/Originalidade) + a sugestão de correção viram parte do julgamento do **Revisor**, na mesma etapa/decisão que já existe. Aplica-se a qualquer propriedade, com rigor proporcional ao risco do nicho — pesado em financeiro e saúde (site do irmão do Luiz), leve em decoração (site da mãe dele, não é YMYL).
- **Fase 4b — Geração de imagens (capa + secundárias)**, specs prontas de 19/08/2026 (`docs/insumos-externos/especificacao_automacao_capa_post_persona.md` e `especificacao_automacao_imagens_secundarias_post.md`). Duas automações completas e detalhadas: capa foto-realista cruzando post + persona (5 etapas: resumo do post, resumo psicológico da persona, cruzamento, prompt, geração), e até 3 imagens secundárias informativas com filtro rígido de utilidade (nota ≥30/40, zero imagens sendo resposta válida) e protocolo anti-alucinação ("banco fechado" — a imagem só pode conter o que está literalmente no trecho-fonte do artigo). Ambas concordam com a auditoria (item acima) e com decisão do Luiz: imagem só é gerada **depois** do texto aprovado pelo Revisor, nunca antes — essa parte sim é uma etapa nova, genuinamente pós-aprovação. Escopo real: novos agentes de IA (resumo de post, resumo de persona, cruzamento, identificador de oportunidades visuais, gerador, **revisor de imagem** — pedido do Luiz, 19/08/2026, foco principal em pegar **alucinação** e **conteúdo divergente do post** — a imagem afirmando algo que o trecho-fonte não sustenta, mesmo princípio de "banco fechado" das duas specs aplicado à validação, não só à geração), upload de mídia no Adaptador WordPress (não existe hoje), e a lógica de sequenciamento pós-aprovação.
  - **Fornecedor decidido (19/08/2026, após comparação de custo):** Anthropic (Claude Sonnet 5) pra todo o raciocínio em texto/visão (resumos, cruzamento, revisor de imagem — mesmo modelo já usado no resto do pipeline) + OpenAI (GPT Image 2) só pra gerar os pixels, já que a Anthropic não tem modelo de geração de imagem. Estimativa: ~R$3,85/post em qualidade alta ou ~R$1,27/post em qualidade média pra capa + 3 secundárias (teto — a maioria dos posts deve sair com 0-1 secundária aprovada, dado o filtro rígido das specs), sem contar o custo do texto em si (Escritor+Revisor, ~R$0,50/post quando aprova de primeira). Câmbio e preços de API mudam — reconferir antes de orçar de verdade.
- **Fase 4c — Distribuição multi-canal**, o "estágio 4" do desenho original (17/08/2026, seção 3.3 acima) que ficou fora da Fase 1 construída — trazido de volta pro planejamento em 19/08/2026 a pedido do Luiz, como parte do mesmo fluxo completo ponta a ponta. Roda estritamente **depois** da publicação real no WordPress (precisa da URL final do post). Dois padrões, já mapeados na seção 3.3: **resumo + CTA + imagem recortada** (Google Meu Negócio, Instagram, Facebook, LinkedIn feed, Pinterest, sincronização de descrição no YouTube quando existir vídeo relacionado) e **republicação integral com `canonical`** (Medium, LinkedIn Articles). Fora do pipeline automático por decisão de escopo (seção 3.3): Quora, Reddit, HARO Brasil — exigem curadoria humana, ficam de fora. Escopo real: adaptadores novos por canal (`src/lib/marketing/canais/`, só WordPress existe hoje — Google Business Profile API, Meta Graph API, LinkedIn API, Pinterest API), recorte/redimensionamento de imagem por formato (determinístico, não geração nova), e as credenciais de cada canal (seção 8, a maioria ainda não provisionada).
  - **Gap achado pelo Luiz nas specs (19/08/2026), nenhuma das duas cobre:** metadados de SEO/AEO da própria imagem — ALT text (obrigatório, derivado do mesmo trecho-fonte usado pra gerar a imagem, nunca inventado à parte — quebraria o princípio de "banco fechado"), slug/nome de arquivo descritivo com palavra-chave em kebab-case (não hash aleatório — Google Imagens lê o nome do arquivo), title attribute, legenda visível (`<figcaption>`) nas secundárias (funciona como texto citável extra, mesmo espírito da "resposta direta abaixo de cada H2" já no checklist), e a capa alimentando o campo `image` do Schema `Article` (pendência de dados estruturados, item acima) — precisa entrar nas specs antes de implementar.
- **Fase 4d (ideia registrada, não decidida) — atualização de posts já publicados no WordPress**, levantada pelo Luiz em 19/08/2026: depois do motor de conteúdo pronto (Fase 4a-c), importar os posts ativos já publicados e restruturá-los/modernizá-los pros mesmos requisitos de SEO/AEO/autoria — não reescrever do zero. **Avaliação crítica registrada (não é consenso automático, é risco real dos dois lados):**
  - **A favor:** os posts antigos provavelmente têm o mesmo tipo de risco YMYL achado na amostra nova em 19/08 (confusão entre prescrição de dívida e prazo de negativação) — só que ao vivo há mais tempo, com exposição legal/reputacional real. Corrigir isso é redução de risco, não só SEO. Backfill de autoria/schema estruturado em conteúdo existente é praticamente só metadado — baixo risco, alto valor de E-E-A-T.
  - **Contra:** atualizar `dateModified` de muitos posts em lote, num período curto, via automação, é o padrão que a política de spam do Google "scaled content abuse" (desde a atualização de março de 2024) foi desenhada pra detectar — independente da intenção real ser boa. E post existente já tem ranking/backlinks acumulados; reestruturar mexe em algo que já funciona, risco só existe pra baixo (post novo só tem risco pra cima).
  - **Recomendação registrada, não decisão:** separar as duas coisas — (1) backfill de autoria/schema em tudo, baixo risco, pode ser amplo; (2) reestruturação de conteúdo só depois de auditar (sem publicar) com o Revisor estendido da Fase 4a em modo leitura, corrigindo primeiro só o que tem risco factual real, espaçado ao longo do tempo, nunca em lote único.
  - **Dado que falta pra decidir com informação, não no escuro:** o Luiz confirmou que já existem credenciais do Search Console e do Analytics — antes de tocar em qualquer post existente, puxar os números reais (tráfego, ranking, impressões por post) pra priorizar o que já funciona (não mexer) do que está fraco ou arriscado (candidato real). Não iniciar sem essa etapa.
  - **Não bloqueia nem faz parte do escopo desta Fase 4** (a-b-c) — só faz sentido depois do motor de conteúdo estar pronto e testado. Fica registrado aqui pra revisão futura.
- **Fase 4e (gap achado pelo Luiz, 19/08/2026) — Agente Agendador, faltou desde o desenho original.** Hoje o pipeline publica DIRETO no WordPress assim que o post é aprovado (`adaptador.aprovarPublicar`, `processar-pauta.ts` → `status: "publish"` imediato) — não existe conceito de agenda. O Luiz apontou que isso está errado: cada propriedade precisa de uma agenda real de posts (quantos por dia, em quais horários), e cada post novo deve ser **agendado**, não publicado na hora. Desenho proposto, ainda não construído:
  - **Config por propriedade:** quantidade de posts/dia + horários de liberação. Já existe um `posts_por_dia`/`janela_publicacao` em `config_pipeline` hoje, mas isso é usado só como um GATE ("pode processar mais uma pauta agora?" — `cotaDiariaAtingida`/`dentroDaJanela`, `processar-pauta.ts`), não como uma agenda de quando cada post individual vai ao ar. Precisa virar de fato uma agenda (ex.: 1 post/dia às 09h, ou 2 posts/dia às 09h e 15h) que o Agendador consulta pra escolher o próximo horário livre.
  - **Novo agente "Agendador"**, etapa nova entre "gerar_imagens" e "publicar": depois que o post está pronto (texto aprovado + imagens embutidas), decide a data/hora de liberação respeitando a agenda da propriedade e os posts já agendados pra não empilhar dois no mesmo horário.
  - **Postador (Adaptador WordPress) usa o suporte NATIVO do WordPress a posts agendados** — `status: "future"` + campos `date`/`date_gmt` no futuro em vez de `status: "publish"` imediato (`criarRascunho`/`aprovarPublicar`, `src/lib/marketing/canais/wordpress.ts`). O próprio WordPress libera o post automaticamente no horário certo — não precisa de um cron nosso pra "soltar" o post depois.
  - **Consequência pra Fase 4c (distribuição multi-canal):** ela já era desenhada pra rodar só "depois da publicação real" — com o Agendador, isso fica mais rígido: só faz sentido disparar GMB/LinkedIn/redes sociais depois que o post **realmente está no ar** (a data agendada já passou), não no momento em que foi enviado ao WordPress com `status: "future"`. Provavelmente exige um mecanismo de verificação próprio (ex.: um cron que checa periodicamente se algum post agendado já passou do horário e só então dispara a distribuição) — o pipeline principal roda e termina bem antes do horário agendado chegar, então "publicar" não pode mais ser o gatilho direto da distribuição.
  - **Não implementado ainda, registrado só como pendência** — o Luiz pediu explicitamente pra documentar e tratar depois, não faz parte do escopo da Fase 4a+4b em andamento.
