...
- **Fase 4b — Geração de imagens (capa + secundárias)**, specs prontas de 19/08/2026 (`docs/insumos-externos/especificacao_automacao_capa_post_persona.md` e `especificacao_automacao_imagens_secundarias_post.md`). Duas automações completas e detalhadas: capa foto-realista cruzando post + persona (5 etapas: resumo do post, resumo psicológico da persona, cruzamento, prompt, geração), e até 3 imagens secundárias informativas com filtro rígido de utilidade (nota ≥30/40, zero imagens sendo resposta válida) e protocolo anti-alucinação ("banco fechado" — a imagem só pode conter o que está literalmente no trecho-fonte do artigo). Ambas concordam com a auditoria (item acima) e com decisão do Luiz: imagem só é gerada **depois** do texto aprovado pelo Revisor, nunca antes. Escopo real: novos agentes de IA (resumo de post, resumo de persona, cruzamento, identificador de oportunidades visuais, gerador, **revisor de imagem** — pedido do Luiz, 19/08/2026, mesmo princípio de "banco fechado" aplicado à validação), upload de mídia no Adaptador WordPress (não existe hoje), e a lógica de sequenciamento pós-aprovação.
