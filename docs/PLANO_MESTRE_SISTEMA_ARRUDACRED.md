...
- 🔶 **Bloco C / Fase 5 — IA real, em andamento (17/08/2026):** módulo de IA único compartilhado por todo o sistema (interpretação de checkpoint, resumo de conversa ao assumir, detector de objeção + resposta sugerida, assist do composer, transcrição de áudio, detecção de troca de assunto).
  ...
  - ⬜ **Ainda não construídos:** resumo de conversa ao assumir, detector de objeção + resposta sugerida, assist do composer (Sonnet), transcrição de áudio, detecção de troca de assunto.
- 🔶 **Suporte a "pacote" (N CPF/CNPJ por Oportunidade) — decisão de arquitetura fechada, Fase 1 construída (17/08/2026):** achado real testando a interpretação por IA — o lead "João" mencionou "devo 10 mil" na abertura (corrigido, ver mais abaixo) e depois, ao ser perguntado CPF/CNPJ, a possibilidade de pacote (2+ documentos) veio à tona. O script original (`SCRIPT_LIMPANOME_SERASA_SPC.md`, Passo 4) já previa isso ("é uma lista de itens, não valor único") mas nunca virou engenharia — ficou esquecido.
  ...
  - **Plano em 5 fases:** (1) dados — `oportunidade_documentos` ✅, (2) motor — capacidade de "lista de documentos" + loop da faixa de valor por item (peça mais arriscada, ainda não existe no motor), (3) IA de esclarecimento (3ª saída além de reconheceu/não-reconheceu), (4) precificação somada + oferta de call pro gerente quando é pacote, (5) reescrever texto do script (Passo 4 em diante).
  - ✅ **Fase 1 — dados (17/08/2026):** migration [`20260817060000_oportunidade_documentos.sql`](../supabase/migrations/20260817060000_oportunidade_documentos.sql) — tabela `oportunidade_documentos` (tipo_documento, documento, faixa_valor/faixa_valor_detalhe/valor_aproximado/valor_restricao_estimado/alto_valor por item, FK cascade pra oportunidades) + repositório `src/lib/motor-fluxo/oportunidade-documentos.ts` (CRUD + `recalcularValorEstimadoOportunidade` que soma os itens em `oportunidades.valor_estimado`). **Motor de fluxo ainda não usa nada disso** — só a base de dados existe, ligar de verdade é a Fase 2.
  - ⬜ **Pendente de ação manual de Luiz:** rodar a migration [`20260817060000_oportunidade_documentos.sql`](../supabase/migrations/20260817060000_oportunidade_documentos.sql) no SQL Editor do Supabase.
