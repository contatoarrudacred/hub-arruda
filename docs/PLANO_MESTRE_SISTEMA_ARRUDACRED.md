...
- ✅ **Conteúdo definido e revisado com Luiz em quatro rodadas:** voz da Malala (primeira pessoa, continuidade com o WhatsApp), curto e pessoal — saudação, bloco de confiança (nota Reclame Aqui/Google, 5.000+ clientes, contrato formal, certificado RA1000 + indicação ao Prêmio Reclame Aqui 2026 num item só), vídeo de apresentação institucional do YouTube e capa do post de blog lado a lado (cada um com título/legenda curta e link), CTA "CHAMAR NO WHATSAPP", bloco "fique por dentro" centralizado com ícones + legenda (Site/Whatsapp/Instagram/Facebook/Youtube), rodapé centralizado (site, razão social, CNPJ, descadastro).
- 🔶 **Achado importante — deliverability:** o e-mail de teste da 1ª/2ª rodada não chegou na caixa do Luiz. Investigando o DNS do domínio, o registro **SPF só autoriza o Hostinger** (`v=spf1 include:_spf.mail.hostinger.com ~all`) — **não inclui a Resend**, então o Gmail provavelmente rejeita/filtra como spam por falha de SPF, mesmo com o DKIM da Resend certo (esse está OK, `resend._domainkey` existe). **Ação manual de Luiz:** ajustar o registro SPF do domínio pra incluir a Resend também, mesclando no mesmo registro (só pode existir um TXT de SPF por domínio) — algo como `v=spf1 include:_spf.mail.hostinger.com include:amazonses.com ~all` (Resend roda sobre a infraestrutura da AWS SES). Confirmar a sintaxe exata no painel da Resend (seção de verificação de domínio) antes de aplicar.
