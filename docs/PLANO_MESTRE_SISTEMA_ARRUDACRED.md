...
- ✅ **Bloco B2 — design validado (17/08/2026), construção pendente:** composer estilo WhatsApp Web + card de contato redesenhado — desenho completo (mockups iterados com Luiz) em **`docs/superpowers/specs/2026-08-17-bloco-b2-composer-card-contato-design.md`**. Resumo: card ganha avatar com anel colorido (quem controla), nome em negrito só quando não lida, mensagem nunca em negrito, hora/data reformatados ("DD/MM - HH:MM" quando não é hoje), badges de atribuição/etapa/produto/valor reorganizados em 2 linhas cheias com ícones, contador de não lidas com número (não só bolinha), confirmação de entrega/leitura (✓/✓✓/✓✓ azul), favoritar (sobe pro topo, sem "fixar" separado). Composer ganha emoji, altura dinâmica (1→10 linhas), menu "+" de anexo, ícone "Ações" agrupando os atalhos. Depende de migrations novas (`produtos.nome_reduzido`, `conversas.favorita`, contador real de não lidas, status de entrega em `mensagens`) — nenhuma rodada ainda.
