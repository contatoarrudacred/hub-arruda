...
### Fase 6 — Motor de disparo de follow-up 🔶 decisão de negócio pendente (15/08/2026)
...
- ✅ **Cron de disparo** (`src/app/api/cron/followups/route.ts` + `vercel.json`, a cada 5 min) — varre conversas aguardando resposta, dispara o próximo item vencido da agenda respeitando a janela comercial. Não entrega de verdade via WhatsApp ainda (Fase 7) — só grava no histórico da conversa, exatamente como o simulador.
- ✅ **Encerramento automático:** o item de ordem 7 da agenda Padrão (10 dias sem resposta) marca a oportunidade como Perdida, motivo `LEAD PAROU DE RESPONDER` (texto exato confirmado em `SCRIPT_LIMPANOME_SERASA_SPC.md`).
- ⬜ **Decisão de escopo, não esquecimento:** os itens 8-10 da agenda Padrão (nutrição por e-mail, 30/60/90 dias pós-perda) ficam **fora** deste motor por ora — não existe canal de e-mail conectado, e o próprio script já registra que essa fase "precisa de suporte no sistema" ainda por desenhar (sub-estado ou flag "em nutrição"). Quando o módulo de Marketing/nutrição for desenhado, é aqui que ele se encaixa.
- ⬜ **Pendente de ação manual de Luiz:** rodar a migration [`supabase/migrations/20260815150000_motor_followup.sql`](../supabase/migrations/20260815150000_motor_followup.sql) no SQL Editor do Supabase; configurar a variável de ambiente `CRON_SECRET` no projeto Vercel (protege o endpoint do cron contra chamadas externas); e confirmar que o plano do Vercel permite cron a cada 5 minutos (planos gratuitos historicamente limitam a frequência de cron — vale conferir no painel antes de assumir que vai rodar como configurado).
