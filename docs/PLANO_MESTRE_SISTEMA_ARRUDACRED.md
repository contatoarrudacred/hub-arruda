...
- 🔶 **Pendente — logo pra fundo escuro:** Luiz mandou uma versão do logo preparada pra ficar bem em fundo escuro (a atual tem partes pretas que somem contra navy — por isso o cabeçalho ainda está com fundo claro, provisório). Não deu pra salvar o arquivo ainda: ele chegou colado direto na conversa, sem um caminho de arquivo acessível pra eu ler/hospedar. Assim que Luiz disponibilizar o arquivo de um jeito que dê pra acessar (upload em algum link, ou confirmar onde ficou salvo no computador), aplico: logo novo + cabeçalho volta pro fundo escuro (`layout-base.tsx`) + tamanho 70% maior + centralizado, como pedido.
- ✅ **Imagens resolvidas:**
