...
> 📄 Design do módulo Vendas (cadastro Cliente/Fornecedor/Serviço, contrato, assinatura digital, financeiro da venda, handoff pra Operação): `superpowers/specs/2026-08-17-modulo-vendas-design.md` — spec validada com Luiz em 17/08/2026. Sub-frente Cadastro construída (17-18/08), sub-frentes Contrato/Assinatura/Financeiro construídas (18-19/08), redesenho "Nova Oportunidade" + novo vocabulário do Kanban (`superpowers/specs/2026-08-19-vendas-nova-oportunidade-kanban-design.md`) construído e testado em produção (19-20/08), tela "Detalhes da Venda" redesenhada em quadros (`superpowers/specs/2026-08-20-vendas-detalhes-venda-quadros-design.md`) + PDF certificado pós-assinatura + correções reais de produção (20-21/08) — status completo e diário de bugs reais na seção 11 abaixo.
...
**Regra dura de quem roda migration em produção — atualizada em 18/08/2026 (substitui a versão anterior desta seção):** nenhum agente/subagente executa migration no Supabase, sem exceção — nem quando é aditiva e restrita ao próprio módulo (regra de "raio de impacto" que valia até aqui, revogada). Motivo: mesmo qualificada por raio de impacto, a regra anterior ainda deixava o banco de produção mudar sem o Luiz saber no momento — e ele decidiu centralizar. Fluxo agora: o agente escreve o arquivo `.sql` e reserva o timestamp, o **Coordenador de Agentes** (papel novo, ver `docs/COORDENACAO_AGENTES_ARRUDACRED.md`) leva ao Luiz com link, e o **Luiz roda no SQL Editor**. Nada de `supabase db push`, `migration repair`, `psql` ou REST direto — nenhum caminho que escreva no banco por fora desse fluxo. Detalhe completo e tabela de migrations em uso: `docs/COORDENACAO_AGENTES_ARRUDACRED.md` seção 2.
...
Ver a entrada abaixo (20-21/08/2026) pro que veio depois desta — inclusive uma correção sobre a etapa "Gerando Financeiro", que quase foi removida por engano no mesmo raciocínio da fusão acima, mas era um caso diferente.

### ✅ Módulo Vendas — Detalhes da Venda em quadros + correções reais de produção (20-21/08/2026)

Sequência de pedidos do Luiz depois de usar o sistema ao vivo — cada um discutido, confirmado no código antes de mexer (várias vezes a causa raiz não era a que parecia à primeira vista), corrigido e publicado no mesmo dia. Nenhum migration novo precisou ser criado nesta rodada além do que já estava pendente.

**1. Dois bugs reais na Nova Oportunidade, achados pelo Luiz testando:**
- Endereço de uma pessoa **já cadastrada** não vinha pré-preenchido ao buscar por CPF/CNPJ (só nome/e-mail/RG etc. vinham) — como o campo ficava em branco, nada era enviado pro servidor, e o endereço nunca era atualizado. Corrigido: a busca agora traz o endereço salvo também (`buscarRepresentanteCompleto`-like, via `buscarEnderecoPorPessoa`).
- Corrigir o nome de uma pessoa **já cadastrada** na tela não salvava — o campo é editável nos dois casos (novo/existente), mas a edição só era enviada ao servidor pelo caminho de criação (`pessoaNova`), nunca pro de atualização. Corrigido, com o mesmo ajuste replicado em Fechamento de Venda (lá o nome nunca foi editável, então não tinha o bug, mas o tipo compartilhado precisava do campo mesmo assim).

**2. Tela "Detalhes da Venda" redesenhada em quadros** — spec `superpowers/specs/2026-08-20-vendas-detalhes-venda-quadros-design.md` + plano `superpowers/plans/2026-08-20-vendas-detalhes-venda-quadros.md`, construída via SDD (3 tasks: função de busca do representante, expansão da busca de dados em `page.tsx`, reescrita do client component). Cards que antes só apareciam em estágios específicos do Kanban (assinatura, parcelas) passam a ficar sempre visíveis; 4 cards novos (Dados do Cliente, Dados da Venda, Partes do Contrato substituindo o antigo painel de assinatura, Pacote de Documentos). Revisão final de branch (modelo mais capaz) achou 2 Important reais — endereço com campo nulo podia derrubar a página inteira (cast cru do banco não bate com o tipo `string`); venda comissionada mostrava aviso falso de "template faltando" — corrigidos e re-revisados no mesmo dia. Princípio de "drill-down por item" (botão Ver detalhes futuro em parcela/comissão/evento) registrado na spec, propositalmente não implementado ainda.

**3. Refinamento de UX da mesma tela**, pedido depois de ver em produção: ícones (emoji, mesmo padrão do menu lateral) em cada quadro; status de parcela/comissão viram badge colorido em vez de texto puro; borda de atenção (âmbar/vermelha) em Partes do Contrato/Financeiro quando algo precisa de ação; tooltips nos botões "Verificar... agora" e no badge de etapa; PDF do contrato ganhou 3 ações (Ver/Baixar de verdade com `Content-Disposition: attachment`/Copiar) em vez de um linkzinho discreto; parcela financeira ganhou Ver+Copiar do link de cobrança (boleto/Pix ou checkout do cartão, sem "Baixar" — não é um arquivo); Pacote de Documentos deixou de ser card próprio e virou seção dentro de Dados da Venda (é dado do serviço contratado, não da venda em si — pedido explícito do Luiz).

**4. PDF certificado pós-assinatura** — achado do Luiz: depois que todos assinam, a Assinafy gera um PDF final com certificado de assinatura embutido, diferente do PDF sem assinatura que subimos originalmente, mas nada no sistema buscava esse arquivo de volta — o link "Ver/Baixar/Copiar PDF" continuava mostrando pra sempre a versão sem assinatura. Corrigido no webhook `document_ready`: baixa o artifact "certificated" da Assinafy (`GET /documents/:id/download/certificated`, endpoint confirmado na doc oficial) e sobrescreve o **mesmo** arquivo no Storage (`upsert: true`) — `contratos.pdf_url` não muda, então qualquer link já copiado antes da assinatura passa a servir o PDF assinado sozinho, sem precisar gerar/reenviar nada.

**5. A saga de "Gerando Financeiro"** — vale registrar o processo, não só o resultado, porque é um exemplo real de correção de rumo dentro do mesmo dia. O Luiz perguntou se o avanço automático de etapa e o Realtime do Painel já cobriam toda transição (sim, confirmado no código) — e no processo de responder, achei (errado) que "Gerando Financeiro" era uma coluna morta no Kanban, igual "Envelopando Assinaturas" tinha sido: nenhum contrato nunca teve esse status gravado de verdade. Removi do Kanban. O Luiz então perguntou, corretamente, por que essa etapa não seguia a mesma lógica de "Emitindo Contrato" (que fica visível com erro quando o envio à Assinafy falha) — e a resposta revelou a diferença real: "Gerando Financeiro" **é** uma etapa genuína (existe uma chamada de verdade à Asaas que pode falhar), só faltava o código *marcar* esse status antes de tentar, coisa que "Emitindo Contrato" já fazia. Sem essa marcação, um erro na Asaas deixava o card rotulado "Aguardando Assinaturas" mesmo já tendo sido assinado por todos — etiqueta enganosa, não uma coluna vazia. Revertida a remoção e corrigido de verdade: `tentarGerarFinanceiro` (`progressao.ts`) agora marca `gerando_financeiro` antes de tentar, igual o padrão já estabelecido. A migration que chegou a ser escrita pra essa remoção (`20260820140000`) foi apagada do repo antes do Luiz conseguir rodar — sem impacto, o `CHECK constraint` já em produção já aceitava esse valor o tempo todo.

**6. Colunas transitórias somem quando vazias** — pedido do Luiz no mesmo fio: "Emitindo Contrato" e "Gerando Financeiro" são etapas automáticas que deveriam ser instantâneas (só ficam com um card parado quando algo falha) — diferente de "Aguardando Assinaturas"/"Aguardando Pagamento" (esperas humanas genuínas, sempre fazem sentido como coluna fixa do Kanban). Agora essas duas só aparecem como coluna no Painel de Vendas quando têm ao menos 1 card (`ehEstagioTransitorio`, novo helper) — uma coluna aparecendo já é o sinal de que algo travou, em vez de espaço permanentemente vazio.

**7. Dois ajustes no editor de Template de Documentos**: `{{tabela_vencimentos}}` só gerava a tabela quando havia mais de 1 parcela — uma venda à vista (1 parcela só) ficava sem tabela nenhuma no contrato; corrigido pra gerar sempre (a função que monta a tabela em si sempre esteve certa, o gate errado estava só no chamador). Novo placeholder `{{quantidade_parcelas}}` — resolve pro número de parcelas, sem a palavra "parcelas" nem "x", pra quem monta o texto do jeito que quiser em volta.

**8. Limpeza cruzada**: todos os `console.log`/`console.error` de debug (`[DEBUG ...]`) que restavam no módulo Vendas foram removidos — vazavam nome/documento/e-mail/entrada de formulário inteira no console do navegador e nos logs do servidor.

**Achado cruzado registrado no quadro-branco (não específico desta sub-frente):** o sistema tem — e vai continuar tendo — um Kanban por módulo (CRM = atendimento, Vendas = funil de venda, Financeiro = cobrança futuro), cada um com seu próprio vocabulário e dono. Regra dura registrada em `docs/COORDENACAO_AGENTES_ARRUDACRED.md` (seção 4.1): nenhum agente mexe em arquivo/tabela/coluna com "kanban" no nome que pertença a outro módulo sem acordo explícito antes.

tsc/eslint limpos e vitest verde (2 falhas pré-existentes do Marketing, sem relação com Vendas) em todas as rodadas desta entrada — commits `497f5d4`, `b734ec8`, `3d81214`→`8535f9c` (spec/plano/SDD Detalhes da Venda), `b087eac` (UX), `f4a06d9` (PDF certificado), `fc57762` (correção Gerando Financeiro), `bd9fe70` (templates).

**Pendências reais no momento desta entrada:** nenhuma migration pendente. Falta o teste ao vivo completo no navegador (Kanban, tela nova, template) e o teste real de ponta a ponta assinando/pagando de verdade — nenhum dos dois é possível rodar de dentro do worktree isolado. Ver `docs/status/vendas.md` pro estado corrente, atualizado com mais frequência que esta seção.

### ✅ Módulo Vendas — Painel Interativo por estágio, reconciliação de pagamento e auditoria completa do módulo (21-22/08/2026)

Dia inteiro de testes reais em produção pelo Luiz, cada achado discutido e corrigido na hora — volume grande o bastante pra merecer sua própria entrada, separada da anterior.

**1. Redesenho do Painel Interativo (topo de Detalhes da Venda) + Partes do Contrato + Financeiro** (commit `b91df4e`): a etapa atual passou a vir em destaque (ícone + descrição visível, antes só tooltip) com sugestão de ação por etapa; ganhou as mesmas ações do menu flutuante do Painel de Vendas (Cancelar venda + "Excluir", novo). Partes do Contrato: enquanto não assinado, só mostra o botão de sincronizar com a Assinafy; depois de `assinado_em` preenchido, mostra a data direto do banco, sem nova consulta. Financeiro: boleto/pix ganhou botão de baixar o boleto (`Cobranca.bankSlipUrl`, confirmado na doc oficial da Asaas antes de codar) e os links passaram a carregar sozinhos ao abrir a tela; cartão ganhou quadro próprio com o link do Checkout salvo (migration `20260821100000` — 3 colunas novas em `contratos`, sem isso o link gerado automaticamente nunca ficava salvo em lugar nenhum).

**2. Cadeia de bugs reais, cada um exposto pela correção do anterior** — vale registrar a sequência, não só o resultado: (a) migration `20260821100000` ficou esquecida sem aplicar enquanto o código que já lê essas colunas foi ao ar — tela quebrou, Luiz rodou o SQL; (b) uma venda já paga mostrava o PDF sem assinatura — causa: avançada pelo botão manual de confirmar assinatura antes do fix do item 1 existir; corrigido com um botão isolado "Ressincronizar PDF assinado", que só troca o arquivo sem repetir efeitos colaterais (etapa/cobrança); (c) uma parcela sincronizada com a Asaas continuava com status `previsto` — `atualizarParcelaAsaas` nunca gravava `status`, só o id da cobrança; corrigido pra gravar `'gerado'` junto; (d) essa mesma correção fez o quadro Financeiro tentar consultar a Asaas de verdade pela primeira vez — e expôs que a consulta mandava `parcela.id` (nosso id interno) em vez do id real da cobrança, dando 404. As quatro corrigidas no mesmo dia.

**3. Baixa manual "Recebido em dinheiro"** (`POST /v3/payments/{id}/receiveInCash`, confirmado na doc oficial antes de codar): cliente que paga em espécie fora da Asaas, disponível só na 1ª parcela (decisão do Luiz — evita dar baixa fora de ordem), no quadro Financeiro e no Painel Interativo. Esse caminho não passa pelo webhook normal (a Asaas usa um status diferente pra isso) — motivou extrair `concluirVenda`/`deveConcluirAoConfirmarParcela` (`src/lib/vendas/conclusao-venda.ts`, com teste Vitest de verdade no gate) de dentro do webhook, reaproveitada nos dois caminhos.

**4. Comunicação com o cliente desativada no Vendas, decisão do Luiz:** o módulo mandava WhatsApp/e-mail direto (Zapster/Resend) sem deixar rastro na ficha da Pessoa — quem monta essa timeline é o CRM (`mensagens`/`conversas`), e o Vendas nunca gravava nada ali. `enviarWhatsapp`/`enviarPorEmail` agora lançam erro em vez de enviar; os botões continuam na tela (pedido explícito dele) — clicar mostra a mensagem explicando a situação. O Luiz vai coordenar direto com o agente do CRM como isso deve funcionar e trazer as instruções pra implementar depois — nota registrada no quadro-branco pro CRM ver antes de precisar perguntar.

**5. Spec + plano da reconciliação do Checkout de cartão desenhados** (`docs/superpowers/specs/2026-08-21-vendas-checkout-cartao-recebiveis-design.md` + plano de 9 tasks, subagent-driven-development) — hoje nenhuma venda por cartão conclui sozinha (o webhook só escuta pagamento por parcela, o Checkout nunca gera esse id) e não existe registro das parcelas reais que a Asaas repassa. Aprovado pelo Luiz, ainda não iniciado.

**6. Achados menores da mesma sequência:** `{{tabela_documentos}}` no editor de templates só gerava com mais de 1 documento no pacote (mesmo padrão de bug já corrigido em `{{tabela_vencimentos}}`) — corrigido pra gerar com 1 também; parcela paga deixou de mostrar boleto/copiar/reenvio (só "Ver fatura" continua fazendo sentido); Kanban do Painel de Vendas ganhou a soma do valor parado por coluna, sem emoji e sem o texto "parado aqui" nas colunas terminais (Concluída/Cancelada).

**7. Auditoria completa do módulo (código + documentação), a pedido explícito do Luiz** — revisão sem corrigir nada até apresentar cada achado com sugestão e ele decidir. Aprovados e corrigidos: `cancelarVenda` nunca sincronizava `oportunidades.etapa_kanban` pra "perdida" (diferente do caminho automático de recusa de assinatura, que já fazia isso); `excluirVenda` deixava o PDF órfão no Storage; e o fechamento de 4 pendências antigas registradas desde 19/08 (Leitor de Documento IA perdendo dado extraído em falha de rede; debounce de busca por documento não cancelado; `maxDuration` faltando em 3 páginas — achado um terceiro ponto de entrada além dos 2 já suspeitos — que disparam a cadeia pesada Puppeteer+Assinafy; nota do `ASAAS_API_KEY` confirmada resolvida na prática). Documentação corrigida junto: uma linha de migration no quadro-branco que ficou marcada "aguardando" mesmo depois de aplicada, e esta própria entrada (o Plano Mestre tinha ficado um dia inteiro sem registro).
...
### Negociação de detalhe de pagamento — checkpoint novo no script (18/08/2026)
Pedido do Vendas (captura de forma/parcelas/vencimento pro contrato) resolvido do lado do motor de fluxo: novo checkpoint `negociacao_pagamento` (`ln_passo16_1` reformulado) — cálculo puro de vencimentos (TDD), interpretador especializado de IA, campos derivados com defaults sensatos em `dados`, mensagem de confirmação dinâmica (`criarResolverMensagensDinamicas`) e ligação nos pontos de entrada reais (webhook + simulador). Testado manualmente no simulador; patch SQL pra sincronizar `ln_passo16_1` com o checkpoint novo entregue a Luiz (rodado por ele).

### `ln_passo6` vira menu fechado de faixas, com confirmação em 2 rodadas (18/08/2026)
Redesenho do checkpoint que pergunta a faixa de valor de cada documento (pacote) — antes texto livre puro, agora um **menu fechado com cabeçalho fixo** listando as faixas conhecidas (rótulo dinâmico por faixa, gerado a partir de `faixasPrecos`). `interpretar-faixas-documentos.ts` virou uma fábrica (`criarInterpretadorFaixasDocumentos(faixasPrecos)`) com lógica de **2 rodadas**: rodada 1 escolhe a faixa no menu, rodada 2 confirma (ou corrige) antes de fechar o checkpoint — só cai no `modo_livre` (texto livre + IA) quando a resposta não bate com nenhuma opção do menu nas duas rodadas. Tasks A-E, ver lista de tasks concluídas na sessão.

### Frente "corrigir tudo de forma global" — buffer de mensagens + memória entre turnos (18-19/08/2026)
Luiz pediu, depois de testar o pacote (documentos/faixas) ao vivo: parar de tratar cada mensagem do WhatsApp como um evento isolado sem memória do que já foi dito na mesma "rajada" ou no mesmo checkpoint. Três frentes paralelas:
- ✅ **Buffer/debounce de mensagens rápidas do lead** — `conversas.buffer_token` (migration) + módulo puro de concatenação de mensagens + `persistencia.ts` busca todas as mensagens desde o corte do buffer + gate de debounce no webhook antes de rodar o motor. Efeito: se o lead manda 2-3 mensagens em sequência rápida (comum no WhatsApp — "seguinte" bate enter, digita mais), o motor processa como uma resposta só, na ordem certa, em vez de rodar o parser em cada fragmento isoladamente.
- ✅ **`interpretar-lista-documentos.ts` ganha memória entre turnos** — antes, se o lead respondia o pacote em 2 mensagens (ex.: "2 CPF" depois "e um CNPJ"), a segunda resposta reiniciava a interpretação do zero, perdendo o que já tinha sido dito. Reescrito pra acumular estado entre chamadas + prompt revisado (menos hesitante em pedir esclarecimento repetido).
- ✅ **`modo_livre` de `interpretar-faixas-documentos.ts` ganha o mesmo tratamento** — acumula valores parciais capturados entre turnos (`_faixas_parcial_valores`, codificado/decodificado por funções puras testadas) em vez de reperguntar tudo de novo a cada rodada; prompt também revisado.

### 2º round de bugs reais de produção, achados testando ao vivo (19/08/2026)
- 🐛 **Mensagens fora de ordem na Tela de Atendimento** — inserção em lote (`registrarTurnoMalala`) gravava todas as mensagens de um turno com o mesmo `enviado_em` (default `now()` do banco, sem diferença de milissegundos entre linhas do mesmo INSERT) — a ordenação por timestamp virava um empate resolvido arbitrariamente pelo Postgres. Corrigido: cada linha grava `enviado_em` com um incremento artificial (`agora + índice × 50ms`), garantindo ordem estável igual à ordem de envio.
- 🐛 **Loop infinito no `modo_livre`** — consequência direta de `interpretar-faixas-documentos.ts` **não ter memória entre turnos** antes da frente acima: a cada resposta parcial do lead, o checkpoint reiniciava do zero e reperguntava a mesma coisa, sem nunca fechar. Resolvido pela reescrita de acumulação de estado (frente anterior) — registrado aqui como o bug real que motivou a frente, não uma correção adicional.
- ✅ **Mitigação adicional de entrega fora de ordem** — `BUFFER_POS_MIDIA_MS` (2,5s) inserido em `enviarSequenciaWhatsapp` depois de qualquer envio de mídia, antes do próximo item da sequência (reforça a mitigação já registrada na Fase 7 pro mesmo problema de entrega da Zapster em modo não oficial).

### Reset de conversa — 3 correções em sequência, testando com dado real (19/08/2026)
`/admin/reset-conversa` (ferramenta de teste, Fase 7) quebrou 3 vezes em produção ao ser usada de verdade — cada quebra corrigida na hora:
1. **FK violation ao resetar pessoa com contrato/comissão emitidos** — a ferramenta tentava apagar tudo incondicionalmente; `contratos`/`comissoes_fornecedor_receber` bloqueiam de propósito (decisão já registrada, não cascateiam). Primeira tentativa de correção (apagar contrato/comissão junto) foi **rejeitada por Luiz**: registro fiscal de venda real nunca pode ser apagado. Reescrito pro padrão correto: **bloqueia e avisa**, oferecendo como alternativa segura "apagar só a conversa" (mantém pessoa/contratos intactos).
2. **`.maybeSingle()` quebrando com "multiple rows returned"** — achado real ao testar: o número de teste do próprio Luiz batia em **2 cadastros de pessoa diferentes** (webhook do WhatsApp criou um, Nova Oportunidade criou outro) porque `pessoas.whatsapp` **nunca teve `unique`** no schema. Reset passou a aceitar N pessoas encontradas (bloqueia com lista de nomes em vez de estourar erro).
3. **Mesmo bug no caminho de produção real** — `carregarOuCriarConversaWhatsapp` (o webhook que atende toda mensagem recebida) tinha o mesmo `.maybeSingle()` frágil; sem correção, a Malala simplesmente **parava de responder** pra qualquer número duplicado. Corrigido com o mesmo tratamento defensivo (prefere a pessoa com conversa de WhatsApp ativa entre as N encontradas).

**Gap de dado ainda em aberto, registrado no `COORDENACAO_AGENTES_ARRUDACRED.md` e no inbox do Vendas (19/08/2026):** `pessoas.whatsapp` sem `unique` é a causa raiz — decisão pendente do Luiz (adicionar `unique` de verdade, com o que fazer com duplicatas já existentes, vs. manter defensivo em todo ponto de entrada). Vendas confirmou ciência, dedup por telefone entra na fila deles, não bloqueia nada agora.

### `loading.tsx` global do admin (19/08/2026)
`src/app/admin/(shell)/loading.tsx` — spinner central simples, cobre toda troca de rota em `/admin/*` via convenção de arquivo do App Router (nenhum código de página precisou mudar). Motivado por trocas de tela sem feedback visual durante o teste ao vivo do reset de conversa.

### Incidente Zapster — conexão caiu com QR code pendente (19/08/2026)
Durante teste ao vivo, a Malala parou de responder — diagnóstico feito por evidência, não suposição: rota do webhook (405/401) descartada, tabela `mensagens` vazia pro período (nada chegou), consulta direta à API da Zapster (`GET /wa/instances/{id}`) confirmou `status: "disconnected"` com QR novo pendente. **Não é bug nosso** — infraestrutura de sessão/pareamento da própria Zapster (hipótese inicial de bloqueio por uso intenso de teste foi levantada e **descartada** depois de 4 contra-argumentos concretos de Luiz — o mesmo número conectava normalmente numa plataforma concorrente ao mesmo tempo). Resolveu sozinho horas depois.

**Decisão de resiliência de provedor, registrada mas não iniciada:** Luiz perguntou sobre um adapter que permita trocar de provedor (Zapster → concorrente) se isso voltar a acontecer em produção. WABA oficial **descartado por custo** ("muito cara" por enquanto). Pesquisa comparativa feita (Z-API, Evolution API, Whapi.Cloud, WaSenderAPI, Maytapi) — critérios do Luiz: múltiplas instâncias + documentação `llms.txt`. **Evolution API** (open-source, self-hosted) desponta como candidato mais forte pra rodar como backup na Hostinger (template Docker pronto, plano KVM 1 já cobre o mínimo de recursos). **Não iniciado** — decisão explícita de não construir a abstração até existir um segundo provedor concreto escolhido pra testar ao lado do Zapster (evita abstração especulativa). Pergunta em aberto: testar a estabilidade da Evolution API por uns dias antes de investir tempo de dev, ou já começar o desenho do adapter.

### Tela `/admin/pessoas` (CRUD de Pessoa) — avaliada e adiada por Luiz (19/08/2026)
Luiz pediu uma tela de admin pra listar/ver/editar/incluir/excluir (exclusão só admin) na tabela `pessoas`, no estilo da tela Nova Oportunidade do Vendas. Investigação feita (confirmado: não existe tela assim hoje; `/admin/fornecedores` é o molde mais próximo pro CRUD lista+form, os campos de captura de pessoa de Nova Oportunidade são o molde pro formulário; `usuarios_sistema.nivel_acesso` existe no schema mas **não é lido em lugar nenhum do código** — não há hoje nenhum controle de nível de acesso implementado, então "só admin" exigiria construir esse controle pela primeira vez) — discutido com Luiz antes de especificar, e **ele decidiu adiar** ("acho que não precisa dessa tela não... se precisar peço novamente"). Nenhum código escrito.
...
| 6 | ✅ **RESOLVIDA em 18/08/2026** — o Luiz corrigiu o SPF; `arrudacred.com.br` passou a autorizar o Resend. Destrava a cópia real dos 3 e-mails de nutrição (30/60/90 dias), que continuam em modo simulado. ~~**SPF do domínio não autoriza o Resend/AWS SES** — só autoriza o Hostinger hoje; provável causa dos primeiros e-mails de teste não chegarem | DNS do domínio (fora do código) | Deliverability do e-mail de boas-vindas e da futura nutrição por e-mail — ação exclusiva de Luiz (painel de DNS do registrador/Hostinger) |
| 7 | ✅ **RESOLVIDA em 18/08/2026** — subdomínio criado e testado pelo Luiz; tabela `cliques_rastreio` confirmada no banco, gravando de verdade. Origem do lead passou a ser dado disponível pro Kanban/KPIs (CRM) e pro laço conteúdo→clique→lead (Marketing). ~~**Rastreio de cliques (`zap.arrudacred.com.br`)** — código pronto (`hostinger-zap/index.php`, migration `20260816020000_cliques_rastreio.sql`, correlação no webhook), falta Luiz criar o subdomínio na Hostinger, subir o arquivo e rodar a migration | `RASTREIO_CLIQUES_WHATSAPP.md` | Não bloqueia nada — captação de origem do lead só começa a funcionar quando publicado |
