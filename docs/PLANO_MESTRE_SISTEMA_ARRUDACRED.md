...
TELA_ATENDIMENTO_ARRUDACRED.md            ← tela de CRM pra acompanhar/assumir atendimento real (WhatsApp) — Blocos A+B em produção (17/08/2026), Blocos C/D pendentes (ver seção 11)
...
- **Menu lateral do admin:** cada item ganha ícone; barra ganha modo contraído/expandido configurável — contraído mostra só ícones por padrão e expande por cima de tudo (overlay) ao passar o mouse, escondendo de novo ao tirar; expandido ocupa espaço fixo no layout sempre. Dúvidas em aberto antes de desenhar: a config é por usuário ou única do sistema? Como funciona em mobile (sem hover)?
...
### Pendências de UX registradas por Luiz (17/08/2026) — composer estilo WhatsApp Web + itens da lista de contatos/timeline, ainda não construídas
Luiz mandou prints do composer real do WhatsApp Web e listou 7 pontos, comparados com o que já está documentado/construído:

1. **Composer estilo WhatsApp Web** — 🆕 fora do plano, precisa ser desenhado: ícone de emoji (nunca foi cogitado em nenhum doc); ícone "+" de anexo à esquerda abrindo um menu flutuante nativo (hoje os atalhos ⚡/💬/📎/🎤/📅 são botões soltos numa barra, não um menu); ícone de microfone posicionado do lado do input (hoje é só mais um botão da barra de atalhos); agrupar nossos atalhos próprios (Próxima etapa/Respostas prontas/Agendar) num ícone único de "Ações". **Mapeamento de viabilidade técnica** (menu de anexo nativo do WhatsApp Web tem 12 opções): Documento/Fotos e vídeos/Câmera/Áudio já funcionam hoje via Zapster (`enviarMensagemMidia`); Contato/Pix já estão modelados em `MensagemEtapa` mas o adaptador (`src/lib/whatsapp/enviar.ts`) lança erro — "ainda não implementado"; Enquete/Evento/Nova figurinha/Catálogo/Cobrar não têm endpoint na Zapster nem modelagem hoje — avaliação preliminar é que são features nativas do WhatsApp Business sem equivalente óbvio num BSP genérico (precisa confirmar direto com a doc da Zapster antes de descartar de vez); "Resposta rápida" do menu nativo mapeia pra nossa feature própria de Respostas Prontas (Fase 6 do Bloco B, já construída).
2. **Composer com altura dinâmica** (1 linha → cresce até ~10 linhas empurrando a conversa pra cima → scroll interno depois disso) — 🆕 fora do plano, hoje é um `<input>` de uma linha só.
3. **Confirmação de entrega/leitura** (checkmarks) — ✅ já registrada como pendência do Bloco D (linha do Bloco D acima, "confirmação de leitura") desde a spec original (`TELA_ATENDIMENTO_ARRUDACRED.md` seção 7/9); só falta priorizar e construir. Depende de a Zapster expor `read_confirmation` e de `mensagens` ganhar campo de status de entrega/leitura (hoje não existe).
4. **Contador numérico de não lidas no card da lista** (hoje é só uma bolinha verde sem número) — 🆕 fora do plano tal como pedido: o desenho original só previa contagem numérica nos **botões de filtro** (Tudo/Malala/Humano/Não lidas), não no card individual, e o modelo de dados hoje trata "não lida" como booleano (`última mensagem é do lead`), não como contagem real de mensagens não lidas — implementar o número exigiria repensar esse modelo (rastrear por atendente até onde cada um já leu, não só "a última mensagem geral").
5. **Data + hora nas mensagens da timeline** (hoje só mostra hora) — 🆕 fora do plano, ganho rápido de UI.
6. **Foto do contato** (lista, cabeçalho da conversa, painel CRM) — ✅ já registrada em detalhe na spec original (`TELA_ATENDIMENTO_ARRUDACRED.md` seção 4, inclusive histórico de fotos via tabela `pessoa_fotos`) e listada no Bloco D ("histórico de fotos do contato"); só falta priorizar e construir.
7. **Opções por contato na lista:** bloquear/desbloquear contato no WhatsApp, fixar conversa no topo, favoritar conversa — 🆕 fora do plano, nenhum dos três estava desenhado. Fixar/favoritar são só modelo de dados + UI (viável). Bloquear/desbloquear depende de a Zapster expor esse endpoint — hoje `src/lib/whatsapp/zapster.ts` só tem envio de texto/mídia e presença ("digitando"), nada de bloqueio; precisa confirmar na doc da Zapster antes de prometer.

**Ainda sem decisão de prioridade/ordem** — itens 1, 2, 4, 5 e 7 são trabalho novo a planejar (provavelmente um "Bloco B.2" ou dentro do Bloco D); itens 3 e 6 já estavam no Bloco D, só reforçados aqui.
...
- Interpretação por IA de verdade (hoje só o encaixe existe, sem chamada real) — Fase 5. **Atualização 16/08/2026: vira o próximo passo de construção, junto com a Tela de Atendimento — ver seção acima.**
