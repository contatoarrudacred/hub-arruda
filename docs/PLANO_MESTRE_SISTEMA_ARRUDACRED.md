...
- 🔶 **Envio real ainda pendente nos dois canais:** WhatsApp aguarda a Fase 7 (Zapster); e-mail aguarda integração com **Resend** (provedor escolhido por Luiz, 15/08/2026) — falta gerar a API key e verificar o domínio `arrudacred.com.br` (registros DNS SPF/DKIM). Até lá, todo disparo (WhatsApp ou e-mail) só registra no banco como se tivesse sido enviado — mesmo princípio usado em todo o MVP1 até a integração real existir.
...
- ⬜ **Pendente de ação manual de Luiz:** rodar as migrations [`20260815150000_motor_followup.sql`](../supabase/migrations/20260815150000_motor_followup.sql) e [`20260815160000_followup_emails.sql`](../supabase/migrations/20260815160000_followup_emails.sql) no SQL Editor do Supabase (a primeira já foi rodada; a segunda, criada depois, ainda não); configurar `CRON_SECRET` no Vercel quando for deployar; criar a conta/API key da Resend e verificar o domínio quando for a hora de ligar o envio real de e-mail.
