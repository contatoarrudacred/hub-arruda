...
tarefa: Sequência de achados reais testando o redesenho da Detalhes da Venda (`b91df4e`) em produção, todos corrigidos no mesmo dia: (1) tela quebrada por migration `20260821100000` esquecida — Luiz rodou o SQL, resolvido. (2) PDF sem assinatura numa venda já paga — botão "Ressincronizar PDF assinado" (`e904939`). (3) Parcela sincronizada com a Asaas continuava mostrando `previsto` — achado um bug real: `atualizarParcelaAsaas` nunca gravava `status`, só o `asaas_payment_id`; corrigido pra gravar `'gerado'` junto (`abf9096`) + SQL de backfill passado ao Luiz pras parcelas antigas. (4) Baixa manual "Recebido em dinheiro" sincronizada com a Asaas (`POST /v3/payments/{id}/receiveInCash`, confirmado na doc antes de codar) — só na 1ª parcela (pedido explícito do Luiz), disponível no quadro Financeiro e no Painel Interativo; extraída `concluirVenda`/`deveConcluirAoConfirmarParcela` (`src/lib/vendas/conclusao-venda.ts`, TDD) do webhook pra ser reaproveitada nesse caminho manual, que não passa pelo webhook normal (commit `06e7a40`). (5) Depois do fix (3), o quadro Financeiro passou a tentar consultar a Asaas de verdade — e expôs outro bug real, também corrigido: `CardFinanceiro` mandava `parcela.id` (nosso id interno) em vez de `parcela.asaasPaymentId` (id real da cobrança) na consulta, dando 404 (commit `bc66667`). Nota registrada no quadro-branco (seção 3) pro futuro módulo Financeiro sobre as funções da Asaas já construídas. Em paralelo, a spec+plano da **reconciliação do Checkout de cartão** (`docs/superpowers/specs/2026-08-21-vendas-checkout-cartao-recebiveis-design.md` + plano de 9 tasks) segue desenhada, aprovada, mas ainda não iniciada. tsc/eslint limpos, vitest 625/627 (2 falhas pré-existentes do Marketing) em todas as rodadas. Tudo empurrado em `vendas-contrato` e `main`.
desde: 2026-08-21T21:45:00-03:00
proxima: **Aguardando o Luiz** — ele vai coordenar direto com o agente do CRM como o envio de WhatsApp/e-mail pro cliente deve passar a funcionar (hoje registra na ficha da Pessoa via `mensagens`/`conversas`, que só o CRM escreve) e depois trazer as instruções de como plugar isso no Vendas. Não iniciar nada disso sozinho antes das instruções chegarem. Também pendente: um contrato assinado por todos não avançou sozinho no Kanban (2ª vez nesta sessão) — investigado, achado que a tela `/admin/configuracoes/assinafy-webhook` só conferia o começo da URL do webhook, nunca o segredo (`?secret=`), podendo mostrar "✅ tudo certo" com o segredo desatualizado; reforçada pra comparar o segredo de verdade (commit `ef41644`) — **Luiz precisa revisitar essa tela e ver se agora ela aponta "Segredo: NÃO bate"**, o que confirmaria a causa raiz de vez. Ajustes de UX também feitos: parcela paga não mostra mais boleto/copiar/reenvio, só "Ver fatura" (`7fa7821`); `{{tabela_documentos}}` no editor de contratos só gerava com mais de 1 documento no pacote (mesmo bug de `{{tabela_vencimentos}}` corrigido antes hoje) — corrigido pra gerar com 1 documento também (`bd8939f`).
