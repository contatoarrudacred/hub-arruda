...
desde: 2026-08-21T21:40:00-03:00
proxima: **Aguardando o Luiz** — ele vai coordenar direto com o agente do CRM como o envio de WhatsApp/e-mail pro cliente deve passar a funcionar (hoje registra na ficha da Pessoa via `mensagens`/`conversas`, que só o CRM escreve) e depois trazer as instruções de como plugar isso no Vendas. Não iniciar nada disso sozinho antes das instruções chegarem. Também pendente: um contrato assinado por todos não avançou sozinho no Kanban (2ª vez nesta sessão) — investigado, achado que a tela `/admin/configuracoes/assinafy-webhook` só conferia o começo da URL do webhook, nunca o segredo (`?secret=`), podendo mostrar "✅ tudo certo" com o segredo desatualizado; reforçada pra comparar o segredo de verdade (commit `ef41644`) — **Luiz precisa revisitar essa tela e ver se agora ela aponta "Segredo: NÃO bate"**, o que confirmaria a causa raiz de vez. Ajuste de UX também feito: parcela paga não mostra mais boleto/copiar/reenvio, só "Ver fatura" (commit `7fa7821`).
