...
tarefa: Detalhes da Venda + Kanban + Templates — depois do redesenho em quadros (commit `11423be`), 5 rodadas de refinamento pedidas pelo Luiz após ver em produção: (1) UX — ícones, badges, bordas de atenção, tooltips, PDF com 3 ações, parcela com Ver+Copiar, Pacote de Documentos virou seção de Dados da Venda (`b087eac`). (2) PDF certificado — webhook `document_ready` agora baixa o artifact "certificated" da Assinafy e sobrescreve o PDF no Storage assim que todos assinam (`f4a06d9`). (3)+(4) Kanban — "Gerando Financeiro" quase foi removida por engano, revertida e corrigida de verdade (`tentarGerarFinanceiro` marca o status antes de tentar, igual "Emitindo Contrato"); "Emitindo Contrato"/"Gerando Financeiro" só aparecem como coluna no Painel quando têm card (`ehEstagioTransitorio`); logs de debug removidos (`fc57762`). (5) Editor de templates — `{{tabela_vencimentos}}` só gerava com mais de 1 parcela (venda à vista ficava sem tabela); corrigido pra gerar sempre. Novo placeholder `{{quantidade_parcelas}}` (`bd9fe70`). tsc/eslint limpos, vitest verde (2 falhas pré-existentes do Marketing) em todas as rodadas.
desde: 2026-08-21T00:40:00-03:00
proxima: Luiz revisar no navegador — Kanban (colunas transitórias somem/aparecem certo), tabela de vencimentos numa venda à vista (1 parcela), o placeholder novo num template; e, principalmente, rodar o teste real de ponta a ponta assinando um contrato de teste de verdade — é o único jeito de confirmar que a troca do PDF certificado e o avanço automático de etapa funcionam contra a Assinafy/Asaas reais (não dá pra simular isso sem as contas configuradas).
bloqueio: nenhuma migration pendente. Falta só o teste ao vivo no navegador e o teste real assinando/pagando de verdade (nenhum dos dois é possível rodar de dentro deste worktree).
