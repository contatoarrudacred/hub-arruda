...
tarefa: Tela "Detalhes da Venda" redesenhada — spec (docs/superpowers/specs/2026-08-20-vendas-detalhes-venda-quadros-design.md) + plano (docs/superpowers/plans/2026-08-20-vendas-detalhes-venda-quadros.md), construída via SDD (3 tasks + revisão final de branch + 1 rodada de correção). Cards que antes só apareciam em estágios específicos do Kanban (assinatura, parcelas) agora ficam sempre visíveis, mais 3 cards novos (Dados do Cliente, Dados da Venda, Pacote de Documentos) — nenhuma tabela/coluna nova, tudo dado que já existia. Revisão final achou 2 Important reais (endereço com campo nulo podia derrubar a página inteira; venda comissionada mostrava aviso falso de "template faltando") — corrigidos e re-revisados, commit `6fa0070`. Publicado em `main`/`vendas-contrato`, commit `11423be`. Princípio de "drill-down por item" (botão Ver detalhes futuro em parcela/comissão/evento) registrado na spec seção 8, propositalmente não implementado.
desde: 2026-08-20T22:40:00-03:00
proxima: Luiz revisar a tela nova no navegador (produto próprio, subcontratado e comissionado, PJ com representante) e confirmar os fixes de endereço/nome da Nova Oportunidade testando com pessoa já cadastrada; depois rodar o teste real de ponta a ponta — Nova Oportunidade → PDF → Assinafy (assinatura de verdade) → Asaas (cobrança/pagamento de verdade).
bloqueio: nenhum — falta teste ao vivo no navegador (Luiz) e o teste real assinando/pagando de verdade.
