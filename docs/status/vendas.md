...
desde: 2026-08-21T19:05:00-03:00
proxima: Luiz testar de novo com uma venda nova (boleto/pix) — verificar se o quadro Financeiro agora consulta certo e mostra fatura/boleto sem erro 404. (5) **Achado e corrigido nesta rodada**: o "id órfão" do 404 não era dado velho — era bug de verdade em `CardFinanceiro` (`detalhes-venda-client.tsx`): a busca de status na Asaas mandava `parcela.id` (nosso id interno) em vez de `parcela.asaasPaymentId` (id real da cobrança). Só ficou visível depois do fix do item (3) acima, porque antes as parcelas nunca saíam de `previsto` pra entrar nesse filtro. Corrigido (commit `bc66667`). Depois de confirmado, retomar o plano da reconciliação do Checkout de cartão (subagent-driven-development, 9 tasks).
bloqueio: nenhum — nenhuma migration nova pendente desta rodada.
