...
tarefa: Rodada de fechamento das 6 pendências levantadas em 22/08/2026: migration `130001` confirmada aplicada, escopo do Checkout de cartão reduzido pro Financeiro (spec+plano atualizados), 3 Minors da auditoria de 21/08 implementados, investigação de código do "Kanban não avançou sozinho" (ver seção abaixo) sem bug encontrado, e — ao revisar o que sobrou pro Vendas na redução de escopo do Checkout de cartão — achado que a parte que continuava sendo nossa (`CHECKOUT_PAID` concluindo a venda) **ainda não estava implementada**, só documentada. Implementado agora: `buscarContratoPorAsaasCheckoutId` (`src/lib/vendas/contratos.ts`) + tratamento do evento `CHECKOUT_PAID` em `src/app/api/webhooks/asaas/route.ts` (acha o contrato pelo `checkout.id`, chama `concluirVenda` já existente, sem esperar parcela). Tudo commitado e já em `main`/`vendas-contrato`.
desde: 2026-08-22T00:00:00-03:00
proxima: **Aguardando o Luiz** gerar uma Oportunidade nova e seguir até a assinatura pra ver se o Kanban avança sozinho dessa vez — se não avançar, ele vai olhar os logs da Vercel/Assinafy antes de qualquer coisa nova ser desenhada. Ideia de reconciliação automática (cron) mencionada e aceita por ele, mas só se o problema virar recorrente — não desenhar sem ele pedir de novo. Também aguardando ele testar uma venda de cartão de verdade em produção pra validar o `CHECKOUT_PAID` recém-implementado (payload real nunca foi visto — só o de `CHECKOUT_CREATED` está confirmado na doc, ver spec seção 7). Depois: aguardar instrução do Luiz sobre comunicação centralizada do CRM (ele disse explicitamente "aguarde minha instrução" — nenhuma ação do Vendas até lá).
...
## Pendências de teste em produção (22/08/2026) — nenhuma bloqueia o módulo, nenhuma foi verificada com dado real ainda

- [ ] **Avanço automático do Kanban após assinatura completa** — Assinafy. Sem bug de código encontrado (ver investigação abaixo); Luiz vai reproduzir numa Oportunidade nova. Se travar de novo, ele confere os logs da Vercel/Assinafy antes de qualquer coisa nova ser desenhada.
- [ ] **Payload real do evento `CHECKOUT_PAID`** — Asaas. Nunca foi visto; só o payload de `CHECKOUT_CREATED` (evento "irmão") está confirmado na doc oficial. Primeira venda de cartão paga de verdade em produção precisa ser conferida no log do webhook (`console.log` já existe em `src/app/api/webhooks/asaas/route.ts`) antes de considerar o fluxo validado ponta a ponta.
- [ ] **Cliente duplicado no painel Asaas via Checkout** — baixa prioridade, sem confirmação oficial na doc, sem impacto funcional (reconciliação usa `externalReference`, não o id do cliente Asaas).
