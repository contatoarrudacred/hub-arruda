...
tarefa: Detalhes da Venda + Kanban — depois do redesenho em quadros (commit `11423be`), 4 rodadas de refinamento pedidas pelo Luiz após ver em produção: (1) UX — ícones, badges, bordas de atenção, tooltips, PDF com 3 ações, parcela com Ver+Copiar, Pacote de Documentos virou seção de Dados da Venda (`b087eac`). (2) PDF certificado — webhook `document_ready` agora baixa o artifact "certificated" da Assinafy e sobrescreve o PDF no Storage assim que todos assinam (`f4a06d9`). (3) Achei (errado) que "Gerando Financeiro" era coluna morta igual "Envelopando Assinaturas" e removi do Kanban — o Luiz apontou a diferença: essa etapa É real (chamada de verdade à Asaas que pode falhar), só faltava o código marcar o status antes de tentar. **Revertido e corrigido de verdade** (`fc57762`): `tentarGerarFinanceiro` agora marca `gerando_financeiro` antes de tentar (mesmo padrão de "Emitindo Contrato"), Kanban volta a 6 estágios + cancelada, migration `20260820140000` (que o Luiz tentou rodar e deu erro) apagada do repo — não precisa rodar nada, o `CHECK constraint` já em produção já aceita esse valor. (4) Pedido extra do Luiz: "Emitindo Contrato" e "Gerando Financeiro" (etapas automáticas que deveriam ser instantâneas) só aparecem como coluna no Painel quando têm pelo menos 1 card — `ehEstagioTransitorio`, novo helper. Aproveitado pra tirar todos os `console.log`/`console.error` de debug que vazavam dado pessoal no console/logs. tsc/eslint limpos, vitest 578/580 (2 falhas pré-existentes do Marketing) em todas as rodadas.
desde: 2026-08-21T00:25:00-03:00
proxima: Luiz revisar a tela e o Kanban no navegador (conferir que "Gerando Financeiro" só aparece quando há card nela, e que "Emitindo Contrato" some quando vazia); e, principalmente, rodar o teste real de ponta a ponta assinando um contrato de teste de verdade — é o único jeito de confirmar que a troca do PDF certificado e o avanço automático pra "Gerando Financeiro"/"Aguardando Pagamento" funcionam contra a Assinafy/Asaas reais (não dá pra simular isso sem as contas configuradas).
bloqueio: nenhuma migration pendente agora (a `120000` já rodou, a `140000` foi retirada — não substituída por outra). Falta só o teste ao vivo no navegador e o teste real assinando/pagando de verdade (nenhum dos dois é possível rodar de dentro deste worktree).
