...
async function gerarCruzamento(
  ...
  const prompt = [
    ...
    "",
    `O AMBIENTE desta cena é: ${ambiente} — use-o SEMPRE, sem exceção nenhuma. Histórico real (22/08/2026): duas versões anteriores desta instrução tentaram abrir uma exceção pra quando "o post/a persona já tem uma cena concreta própria" — e as duas causaram reprovação real do Revisor (próxima etapa) em produção, porque TODA persona deste banco tem uma ficha rápida rica e específica (um galpão, um apartamento, uma cozinha, um objeto na tela do celular...) — sempre existe algum detalhe pra "justificar" trocar o ambiente, o que na prática cancela o sorteio por completo e recria o problema que ele existe pra resolver (capas convergindo pro mesmo tipo de cena óbvia). Por isso: não existe mais exceção. Mesmo que o resumo da persona ou do post mencione um lugar específico, o ambiente desta cena continua sendo ${ambiente}.`,
...
async function gerarPromptCapa(
  ...
  const prompt = [
    ...
    "",
    `AMBIENTE OBRIGATÓRIO DESTA CENA: ${ambiente} — mantenha exatamente este ambiente no prompt final, sem exceção nenhuma (ver a etapa anterior: exceções aqui já causaram reprovação real do Revisor duas vezes em produção, porque toda persona deste banco tem uma cena rica própria pra "justificar" trocar o ambiente — não existe mais essa exceção). O ambiente é só o cenário: o prompt final ainda precisa deixar claro, através de um objeto/gesto/tela/expressão coerente com a IDEIA VISUAL, que a cena é sobre o conflito real deste post — não vire uma foto genérica desse ambiente sem nenhum sinal do assunto do artigo.`,
    "",
    `TÍTULO:\n${titulo}`,
    "",
