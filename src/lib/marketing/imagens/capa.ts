...
async function gerarCruzamento(
  ...
  const prompt = [
    ...
    "",
    `O AMBIENTE desta cena é: ${ambiente} — use-o OBRIGATORIAMENTE, EXCETO numa situação específica: se o RESUMO DO POST abaixo já narrar uma cena concreta com um local/ambiente PRÓPRIO embutido na história (comum em ângulos de storytelling, "antes e depois", ou qualquer post que narre uma situação específica já acontecida — ex.: "ela estava no caixa do supermercado quando o cartão foi recusado"), a fidelidade a ESSA cena real vence o ambiente sorteado. Motivo: a próxima etapa (Revisor de imagem) rejeita qualquer cena que não bata com o que está literalmente escrito no post — forçar um ambiente diferente do que a história já conta garante reprovação nas duas tentativas de geração, sem chegar a lugar nenhum (achado real, 22/08/2026). Fora desse caso específico — a maioria dos posts, que NÃO narram uma cena/local já implícito no texto — o ambiente sorteado continua obrigatório, é ele que resolve as capas saindo sempre parecidas.`,
