...
async function gerarCruzamento(
  ...
  const prompt = [
    ...
    "",
    `O AMBIENTE desta cena é: ${ambiente} — use-o OBRIGATORIAMENTE. A ÚNICA exceção admitida: o RESUMO DO POST abaixo narra, com um verbo de ação num momento específico (não uma explicação geral), uma cena que JÁ ACONTECEU num local explicitamente nomeado — o teste é "o post conta uma HISTÓRIA com cenário próprio, ou só EXPLICA um assunto?". Exemplo real do que conta como exceção: "ela estava no caixa do supermercado quando o cartão foi recusado" (evento narrado, local nomeado). Exemplo real do que NÃO conta como exceção, mesmo remotamente relacionado a lugares/comércio (achado real, 22/08/2026 — a IA já confundiu isso e forçou uma cena de mercado num post que não tinha nenhuma): o post apenas FALAR SOBRE dinheiro, crédito, financiamento, "mercado de crédito", compras, produtos ou pagamento, sem narrar um evento específico com local nomeado. Renda alta, score de crédito, financiamento, PJ, cartão — nenhum desses tópicos, sozinho, é motivo pra trocar o ambiente por uma cena de caixa/loja/banco; isso seria a IA "decidindo" por conta própria a cena mais óbvia pro assunto, exatamente o padrão de convergência que este sorteio existe pra quebrar. Na dúvida entre usar o ambiente sorteado ou trocar por outro, USE O SORTEADO — o padrão é obedecer o sorteio; a exceção é rara e exige uma cena narrada de verdade, não uma associação temática.`,
...
async function gerarPromptCapa(
  ...
  const prompt = [
    ...
    "",
    `AMBIENTE SORTEADO PRA ESTA CENA: ${ambiente} — na maioria dos casos, mantenha exatamente este ambiente no prompt final. EXCEÇÃO: se a IDEIA VISUAL acima já ancorou a cena num local diferente porque o post narra uma cena concreta própria (a etapa anterior tem instrução explícita pra isso) — nesse caso, siga a IDEIA VISUAL, não force o ambiente sorteado por cima dela; a fidelidade à cena real do post (o que o Revisor de imagem, próxima etapa, vai checar) importa mais que o sorteio. Fora dessa exceção, o ambiente é só o cenário: o prompt final ainda precisa deixar claro, através de um objeto/gesto/tela/expressão coerente com a IDEIA VISUAL, que a cena é sobre o conflito real deste post — não vire uma foto genérica desse ambiente sem nenhum sinal do assunto do artigo.`,
    "",
    `TÍTULO:\n${titulo}`,
    "",
