...
/**
 * onMouseDown com preventDefault é essencial: sem isso, o clique no botão tira o foco/seleção do
 * editor ANTES do onClick rodar (mousedown dispara blur no contenteditable), e comandos como
 * "negrito" aplicam no lugar errado (ou em nada) — achado real testando no navegador.
 */
function BotaoBarra({
  ativo,
  aoClicar,
  ...
  return (
    <div className="rounded border border-zinc-300 dark:border-zinc-700">
      <div className="flex flex-wrap items-center gap-1 border-b border-zinc-300 bg-zinc-50 p-2 dark:border-zinc-700 dark:bg-zinc-900">
        ...
        ).map(([alinhamento, rotulo]) => (
          <BotaoBarra
            key={alinhamento}
            ativo={editor.isActive({ textAlign: alinhamento })}
            aoClicar={() => editor.chain().focus().setTextAlign(alinhamento).run()}
          >
            {rotulo}
          </BotaoBarra>
        ))}
        ...
        <BotaoBarra
          ativo={false}
          aoClicar={() => editor.chain().focus().insertTable({ rows: 3, cols: 3, withHeaderRow: true }).run()}
        >
          Tabela
        </BotaoBarra>
